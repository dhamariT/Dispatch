...
- PRs touching 3+ subsystems, >500 lines, or requiring domain-specific expertise (security, concurrency, database, device communication).
...
**Scope the diff.** Get the file list from the diff, PR, or user. Skim for intent and note which layers are touched (API, rollout engine, snapshot engine, fleet monitor, device agent, dashboard, database, auth).

For each changed file, briefly check the surrounding context:

- Config files (package.json, tsconfig, docker-compose, balena.yml, etc.): scan the existing entries for naming conventions and structural patterns.
...
| Database Reviewer    | PostgreSQL/TimescaleDB, data modeling, Go↔SQL boundary      | Migrations, queries, schema, indexes, time-series tables    |
| Security Reviewer    | Auth, attack surfaces, input handling                       | Auth, new endpoints, input handling, API keys, secrets      |
...
| Fleet Reviewer       | Device communication, Balena API, agent protocol            | Agent code, fleet monitor, Balena integration, device auth  |
